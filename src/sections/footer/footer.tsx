...
import TermsModall from "@/components/modalwrapper/termsmodalwrapper";
const Footer = () => {
  const [isOpen, setIsOpen] = useState(false);

  const handlePolicyClick = (itemName: string) => {
    if (itemName.toLowerCase().includes("terms")) {
      setIsOpen(true);
    }
  };
  ...
  return (
    <>
            ...
            {footerLinks.policyLinks.map((item, index) => (
              <li key={index}>
                {item.name.toLowerCase().includes("terms") ? (
                  <button
                    onClick={() => handlePolicyClick(item.name)}
      ...
      <Modal isOpen={isOpen} onOpenChange={setIsOpen} backdrop="blur">
        <ModalContent className="w-full max-w-4xl max-h-[80vh] overflow-y-auto bg-white rounded-lg p-4">
          ...
          <ModalFooter>
            <button
              ...
              className="px-4 py-1 text-white bg-sky-500 rounded hover:bg-sky-600"
            >
              Close
            </button>
          </ModalFooter>
